...
export async function Headers({ username, password, region }) {
...
export async function StoreItems(store) {
  ...
  const res = {
    ...
    itemsTimeLeft:
      store.SkinsPanelLayout.SingleItemOffersRemainingDurationInSeconds * 1000,
    bundleTimeLeft:
      store.FeaturedBundle.BundleRemainingDurationInSeconds * 1000,
  };
